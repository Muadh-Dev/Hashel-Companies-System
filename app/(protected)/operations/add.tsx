...
if (!user?.permissions) {
  // مدير أو بيانات قديمة → كل الخدمات
  SERVICE_TYPES.push("نقل كفالة", "إصدار تأشيرة", "تجديد سنوي")
} else {
  if (user.permissions.sponsorshipTransfer !== "none") {
    SERVICE_TYPES.push("نقل كفالة")
  }
  if (user.permissions.visaIssuance !== "none") {
    SERVICE_TYPES.push("إصدار تأشيرة")
  }
  if (user.permissions.annualRenewal !== "none") {
    SERVICE_TYPES.push("تجديد سنوي")
  }
}
